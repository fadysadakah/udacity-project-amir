...
def combat(weapon):
    ...
    if weapon == "dagger":
        print_sleep(
            f"You feel a bit under-prepared for this, what with only having a tiny {weapon}.", 2)
    ...
    if choice == '1':
        if weapon == "dagger":
            ...
            print_sleep(
                f"The Sword of Ogoroth shines brightly in your hand as you brace yourself for the attack.", 3)
            print_sleep(
        ...
        print_sleep(
            "You run back into the field. Luckily, you don't seem to have been followed.", 2)
        where_to()
...
def intro():
    print_sleep(
        "You find yourself standing in an open field, filled with grass and yellow wildflowers.", 3)
    print_sleep(
        f"Rumor has it that a {enemy} is somewhere around here, and has been terrifying the nearby village.", 3)
    print_sleep("In front of you is a house.", 2)
    print_sleep("To your right is a dark cave.", 2)
    print_sleep(
        f"In your hand you hold your trusty (but not very effective) {weapon}.", 2)
...
def house():
    ...
    print_sleep(
        f"You are about to knock when the door opens and out steps a {enemy}.", 2)
...
def cave():
    ...
    if cave_visited:
        print_sleep(
            "You've been here before, and gotten all the good stuff. It's just an empty cave now.", 2)
